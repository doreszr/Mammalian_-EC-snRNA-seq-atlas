...
### Load Allen snRNA-seq mouse data

### SCRIPT FOR READING LARGE hdf5 DATA SET INTO R

### Creat and save Seurat object for further analysis
...
genes   <- h5read("mouseAllenatlas/v5/expression_matrix.hdf5","/data/gene")
samples <- h5read("mouseAllenatlas/v5/expression_matrix.hdf5","/data/samples")
...
Idents(seu) <- seu$class_label
seu.neu <- subset(x = seu, idents = c("Glutamatergic", "GABAergic"))

##### Normalize with SCTransform #####
seu.neu <- SCTransform(seu.neu, method = "glmGamPoi",verbose = T)

##### Do dimensional reduction #####
seu.neu <- RunPCA(seu.neu, verbose = T)
seu.neu <- FindNeighbors(object = seu.neu, dims = 1:30)
seu.neu <- FindClusters(object = seu.neu)
seu.neu <- RunUMAP(seu.neu, dims = 1:30, verbose = T)

saveRDS(seu.neu, file = "Data_EC_atlas/Seurat_obj/2_mouseAllen/mouse_atlasv5_neu.rds")
